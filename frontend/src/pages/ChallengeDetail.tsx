...
interface ChallengeDetailProps {
  auth: { username: string; email: string; role: string } | null;
  onBack: () => void;
  onLogout: () => void;
  isAccepted?: number[];
...
export function ChallengeDetail({
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      await fetch(`http://localhost:8080/api/challenges/${challenge?.id}`, {
        method: 'DELETE',
        headers: {
            ...
            <div className="flex items-center gap-4">
              {auth?.role === 'ADMIN' && (
                <button
                  ...
                  className="px-3 py-2 bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400 hover:bg-red-200 dark:hover:bg-red-900/50 font-medium rounded-lg transition-colors flex items-center gap-2"
                >
                  <svg
                    className="w-5 h-5"
                    fill="currentColor"
